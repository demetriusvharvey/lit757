...
import { Bell, CalendarDays, ChevronRight, Compass, Heart, MapPin, Music2, Search, ShoppingBag, Sparkles, TreePine, Utensils, Wine, X } from "lucide-react";
...
export default function DesktopHome(){
  ...
  useEffect(()=>{const token=process.env.NEXT_PUBLIC_MAPBOX_TOKEN;if(!mapEl.current||!token||mapRef.current)return;mapboxgl.accessToken=token;const map=new mapboxgl.Map({container:mapEl.current,style:'mapbox://styles/mapbox/dark-v11',center:[-76.17,36.88],zoom:9,minZoom:4,maxZoom:17});map.addControl(new mapboxgl.NavigationControl({showCompass:false}),'top-right');mapRef.current=map;return()=>{map.remove();mapRef.current=null;};},[]);
  useEffect(()=>{const map=mapRef.current;if(!map)return;const render=()=>{const data:GeoJSON.FeatureCollection<GeoJSON.Point>={type:'FeatureCollection',features:filtered.filter(valid).map(v=>({type:'Feature',geometry:{type:'Point',coordinates:coords(v)},properties:{id:v.id,score:score(v)}}))};const src=map.getSource('desktop-venues') as mapboxgl.GeoJSONSource|undefined;if(src){src.setData(data);return;}map.addSource('desktop-venues',{type:'geojson',data,cluster:true,clusterRadius:58,clusterMaxZoom:12});map.addLayer({id:'desktop-clusters',type:'circle',source:'desktop-venues',filter:['has','point_count'],paint:{'circle-color':['step',['get','point_count'],'#7e5cff',15,'#ff9a3d',40,'#ff554a'],'circle-radius':['step',['get','point_count'],15,15,20,40,25],'circle-stroke-color':'#fff','circle-stroke-width':1}});map.addLayer({id:'desktop-count',type:'symbol',source:'desktop-venues',filter:['has','point_count'],layout:{'text-field':['get','point_count_abbreviated'],'text-size':11},paint:{'text-color':'#fff'}});map.addLayer({id:'desktop-pins',type:'circle',source:'desktop-venues',filter:['!', ['has','point_count']],paint:{'circle-color':['interpolate',['linear'],['get','score'],45,'#43d879',65,'#d6dc52',78,'#ff9a3d',88,'#ff554a'],'circle-radius':['interpolate',['linear'],['zoom'],8,4,14,8],'circle-stroke-color':'#fff','circle-stroke-width':1}});map.on('click','desktop-pins',e=>{const id=String(e.features?.[0]?.properties?.id||'');const venue=venues.find(v=>String(v.id)===id);if(venue){setSelected(venue);map.easeTo({center:coords(venue),zoom:14});}});};map.isStyleLoaded()?render():map.once('load',render);},[filtered,venues]);

  const useLocation=()=>navigator.geolocation?.getCurrentPosition(p=>mapRef.current?.easeTo({center:[p.coords.longitude,p.coords.latitude],zoom:14.5,duration:850}));
