...
export default function MobileHome(){
  ...
  useEffect(()=>{
    ...
    const render=()=>{
      ...
      const liveData:GeoJSON.FeatureCollection<GeoJSON.Point>={type:"FeatureCollection",features:allFeatures};
      ...
      if(!venueSource){
        map.addSource("mobile-venues",{type:"geojson",data:allData,cluster:true,clusterMaxZoom:12.5,clusterRadius:72});
        map.addLayer({id:"mobile-clusters",type:"circle",source:"mobile-venues",filter:["has","point_count"],minzoom:7.5,paint:{"circle-color":["step",["get","point_count"],"#f3c94b",18,"#ff9a3d",45,"#ff554a"],"circle-radius":["step",["get","point_count"],11,18,14,45,18],"circle-stroke-width":1.2,"circle-stroke-color":"rgba(255,255,255,.72)","circle-opacity":.92}});
        map.addLayer({id:"mobile-cluster-count",type:"symbol",source:"mobile-venues",filter:["has","point_count"],minzoom:7.5,layout:{"text-field":["get","point_count_abbreviated"],"text-size":9},paint:{"text-color":"#081016"}});
        map.addLayer({id:"mobile-venue-pins",type:"circle",source:"mobile-venues",filter:["!",["has","point_count"]],minzoom:7.5,paint:{"circle-radius":basePinRadius,"circle-color":["interpolate",["linear"],["get","score"],0,"#5f6670",45,"#43d879",58,"#9bd94a",68,"#f3c94b",78,"#ff9a3d",88,"#ff554a"],"circle-stroke-width":["interpolate",["linear"],["get","score"],0,.3,75,.8,90,1.5],"circle-stroke-color":"rgba(255,255,255,.9)","circle-opacity":.96}});
        ...
        const selectMapVenue=(feature?:mapboxgl.MapboxGeoJSONFeature)=>{const venueId=String(feature?.properties?.id||"");const venue=mappedRef.current.find(item=>String(item.id)===venueId);if(!venue)return;setSelected(venue);setSelectedVenueId(venue.id);setDetailsOpen(false);map.easeTo({center:coords(venue),zoom:Math.max(map.getZoom(),13),duration:550});};
        map.on("click","mobile-live-dot",e=>selectMapVenue(e.features?.[0]));
        map.on("click","mobile-venue-pins",e=>selectMapVenue(e.features?.[0]));
  ...
  return <div className="mobile-native-home lg:hidden">
    ...
    <main ref={scrollRef} className="mobile-native-scroll" style={{paddingBottom:78}}>
      ...
      <section className="mobile-native-map" style={mapMode?{position:"fixed",inset:0,zIndex:10010,height:"auto",margin:0,borderRadius:0}:{}}>{mapMode&&<button onClick={goExplore} aria-label="Close full map" style={{position:"absolute",left:14,top:"calc(14px + env(safe-area-inset-top))",zIndex:10,display:"grid",placeItems:"center",width:42,height:42,border:"1px solid rgba(255,255,255,.18)",borderRadius:21,background:"rgba(8,11,16,.92)",color:"white"}}><ChevronLeft/></button>}<div ref={mapEl} className="mobile-native-mapbox"/>{selected&&!detailsOpen&&<div className="map-preview" role="button" tabIndex={0} onClick={()=>setDetailsOpen(true)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" ")setDetailsOpen(true);}}><div className="map-preview-photo"><RemoteVenueImage src={selected.photoUrl} alt="" fallback={selected.name.slice(0,1)} sizes="58px"/></div><div><span className="map-preview-score">{score(selected)}</span><strong>{selected.name}</strong><small>{selected.city||"Nearby"} · {statusFor(selected)}</small><p>{selected.event?.name||selected.reason||"Popular nearby right now"}</p></div><ChevronRight/><button className="map-preview-close" onClick={event=>{event.stopPropagation();setSelected(null);setSelectedVenueId(null);}} aria-label="Close venue preview"><X/></button></div>}<button className="mobile-live-updates-cue" type="button" onClick={()=>feedRef.current?.scrollIntoView({behavior:"smooth",block:"start"})}>🔥 Live updates below <span aria-hidden="true">↓</span></button></section>
    ...
    {favoritesOpen&&<div className="planner-backdrop" onClick={closeOverlay}><section className="utility-sheet" onClick={event=>event.stopPropagation()}><div className="planner-handle"/><div className="utility-head"><div><span>SAVED</span><h2>Your watchlist</h2><p>Places you want LIT757 to watch for you.</p></div><button onClick={closeOverlay}><X/></button></div>{favorites.length?<div className="utility-list">{favorites.map(v=><button key={v.id} onClick={()=>{closeOverlay();openVenue(v);}}><div className="utility-photo"><RemoteVenueImage src={v.photoUrl} alt="" fallback={v.name.slice(0,1)} sizes="64px"/></div><span><strong>{v.name}</strong><small>{statusFor(v)} · Score {score(v)}</small></span><ChevronRight/></button>)}</div>:<div className="utility-empty"><Heart/><strong>No saved places yet</strong><p>Tap the heart on any place to build your watchlist.</p></div>}</section></div>}
