...
export default function MobileHome(){
  ...
  useEffect(()=>{
    ...
    const render=()=>{
      ...
      const liveFeatures:GeoJSON.Feature<GeoJSON.Point>[]=mapped.filter(v=>liveScore(v)>=62&&v.openNow!==false).map((v,index)=>({type:"Feature",geometry:{type:"Point",coordinates:coords(v)},properties:{id:v.id,index,name:v.name,score:liveScore(v)}}));
      ...
      if(!liveSource){
        ...
        const halo:mapboxgl.CircleLayerSpecification={id:"mobile-live-halo",type:"circle",source:"mobile-live-dots",maxzoom:11.2,paint:{"circle-radius":["interpolate",["linear"],["zoom"],7,5,9,7,11.2,9],"circle-color":["step",["get","score"],"#f3c94b",76,"#ff9a3d",88,"#ff554a"],"circle-opacity":0.18,"circle-blur":0.75}};
        const dot:mapboxgl.CircleLayerSpecification={id:"mobile-live-dot",type:"circle",source:"mobile-live-dots",maxzoom:11.2,paint:{"circle-radius":["interpolate",["linear"],["zoom"],7,2,9,2.8,11.2,3.8],"circle-color":["step",["get","score"],"#f3c94b",76,"#ff9a3d",88,"#ff554a"],"circle-opacity":0.95,"circle-stroke-width":1.2,"circle-stroke-color":"rgba(255,255,255,.9)"}};
  ...
  return <div className="mobile-native-home lg:hidden">
    ...
    <main ref={scrollRef} className="mobile-native-scroll" style={{paddingBottom:132}}>
