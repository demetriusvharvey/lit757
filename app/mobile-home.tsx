...
import { Bell, Bookmark, CalendarDays, ChevronLeft, ChevronRight, Compass, Heart, Map, Music2, Search, Send, ShoppingBag, Sparkles, TreePine, Utensils, Wine, X } from "lucide-react";
...
export default function MobileHome(){
  ...
  useEffect(()=>{try{setFavoriteIds(new Set(JSON.parse(localStorage.getItem(FAVORITES_KEY)||"[]") as string[]));setAlertsEnabled(localStorage.getItem(ALERTS_KEY)==="true");}catch{}fetch("/api/discover?city=All%20757&mode=all",{cache:"no-store"}).then(r=>r.json()).then((p:Payload)=>setVenues(p.venues||p.picks||[])).catch(()=>undefined);},[]);
  ...
  useEffect(()=>{
    ...
    const render=()=>{
      ...
      const liveFeatures:GeoJSON.Feature<GeoJSON.Point>[]=mapped.filter(v=>liveScore(v)>=45&&v.openNow!==false).map((v,index)=>({type:"Feature",geometry:{type:"Point",coordinates:coords(v)},properties:{id:v.id,index,name:v.name,score:liveScore(v)}}));
      ...
      if(!liveSource){
        ...
        const activityColor:["interpolate",["linear"],["get","score"],number,string,number,string,number,string,number,string,number,string]=["interpolate",["linear"],["get","score"],45,"#43d879",58,"#9bd94a",68,"#f3c94b",78,"#ff9a3d",88,"#ff6b4f"];
        const halo:mapboxgl.CircleLayerSpecification={id:"mobile-live-halo",type:"circle",source:"mobile-live-dots",maxzoom:11.2,paint:{"circle-radius":["interpolate",["linear"],["get","score"],45,5,65,7,82,9,100,12],"circle-color":activityColor,"circle-opacity":["interpolate",["linear"],["get","score"],45,.08,65,.12,82,.18,100,.25],"circle-blur":.8}};
        const dot:mapboxgl.CircleLayerSpecification={id:"mobile-live-dot",type:"circle",source:"mobile-live-dots",maxzoom:11.2,paint:{"circle-radius":["interpolate",["linear"],["get","score"],45,2,65,2.7,82,3.6,100,4.8],"circle-color":activityColor,"circle-opacity":.96,"circle-stroke-width":["interpolate",["linear"],["get","score"],45,.8,82,1.2,100,1.6],"circle-stroke-color":"rgba(255,255,255,.9)"}};
        map.addLayer(halo);map.addLayer(dot);
      }
      if(!venueSource){
        map.addSource("mobile-venues",{type:"geojson",data:allData,cluster:true,clusterMaxZoom:12,clusterRadius:72});
        map.addLayer({id:"mobile-clusters",type:"circle",source:"mobile-venues",filter:["has","point_count"],minzoom:9.4,paint:{"circle-color":["step",["get","point_count"],"#f3c94b",20,"#ff9a3d",55,"#ff554a"],"circle-radius":["step",["get","point_count"],10,20,13,55,16],"circle-stroke-width":1.5,"circle-stroke-color":"rgba(255,255,255,.82)","circle-opacity":.9}});
        map.addLayer({id:"mobile-cluster-count",type:"symbol",source:"mobile-venues",filter:["has","point_count"],minzoom:9.4,layout:{"text-field":["get","point_count_abbreviated"],"text-size":9},paint:{"text-color":"#081016"}});
        map.addLayer({id:"mobile-venue-pins",type:"circle",source:"mobile-venues",filter:["!",["has","point_count"]],minzoom:11.3,paint:{"circle-radius":["interpolate",["linear"],["zoom"],11.3,4,14,6.5,16,8],"circle-color":["interpolate",["linear"],["get","score"],0,"#5f6670",45,"#43d879",58,"#9bd94a",68,"#f3c94b",78,"#ff9a3d",88,"#ff554a"],"circle-stroke-width":1.4,"circle-stroke-color":"rgba(255,255,255,.9)","circle-opacity":.96}});
  ...
  function closeOverlay(){setFavoritesOpen(false);setAlertsOpen(false);setProfileOpen(false);setActiveTab("explore");}
  ...
  return <div className="mobile-native-home lg:hidden">
    ...
    <main ref={scrollRef} className="mobile-native-scroll" style={{paddingBottom:78}}>
      ...
      <button className="mobile-plan-card" style={{marginTop:18,marginBottom:20}} onClick={openPlanner}><span className="plan-orb"><Sparkles/></span><span><strong>Ask AI</strong><small>Tell us the vibe. We’ll plan the move.</small></span><b>Plan now <ChevronRight/></b></button>
    ...
    {profileOpen&&<div className="planner-backdrop" onClick={closeOverlay}><section className="utility-sheet" onClick={event=>event.stopPropagation()}><div className="planner-handle"/><div className="utility-head"><div><span>PROFILE</span><h2>Your 757</h2><p>Your saved places, alerts, and preferences live here.</p></div><button onClick={closeOverlay}><X/></button></div><div className="alert-card"><div style={{display:"grid",placeItems:"center",width:44,height:44,borderRadius:22,background:"#1a2028",border:"1px solid #424955",fontWeight:800}}>D</div><div><strong>Demetrius</strong><p>{favoriteIds.size} saved places · Alerts {alertsEnabled?"on":"off"}</p></div></div><button className="alert-action" onClick={()=>{setProfileOpen(false);openFavorites();}}>View saved places</button></section></div>}
  </div>;
}
