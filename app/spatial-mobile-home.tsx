...
export default function SpatialMobileHome(){
  ...
  return <div className="mobile-native-home lg:hidden">
    ...
    {favoritesOpen&&<div className="planner-backdrop" onClick={closeOverlay}><section className="utility-sheet" onClick={event=>event.stopPropagation()}><div className="planner-handle"/><div className="utility-head"><div><span>SAVED</span><h2>Your watchlist</h2><p>Places Buzz should watch for you.</p></div><button onClick={closeOverlay}><X/></button></div>{favorites.length?<div className="utility-list">{favorites.map(v=><button key={v.id} onClick={()=>{closeOverlay();openVenue(v);}}><div className="utility-photo"><RemoteVenueImage src={v.photoUrl} alt="" fallback={v.name.slice(0,1)} sizes="64px"/></div><span><strong>{v.name}</strong><small>{statusFor(v)} · Score {score(v)}</small></span><ChevronRight/></button>)}</div>:<div className="utility-empty"><Heart/><strong>No saved places yet</strong><p>Tap the heart on any place to build your watchlist.</p></div>}</section></div>}
