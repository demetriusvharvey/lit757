...
export default function BuzzMobileHome() {
  ...
  useEffect(() => {
    try {
      ...
      // Ignore malformed local preferences and continue with safe defaults.
    }
    void loadSpatial();
  }, [loadSpatial]);
  ...
  useEffect(() => {
    ...
    void loadSpatial({
      ...
      radius,
      label: `within ${radius} mile${radius === 1 ? "" : "s"}`,
    });
  ...
  useEffect(() => {
    ...
    map.addSource("buzz-mobile-venues", {
      type: "geojson",
      data: empty,
      cluster: true,
      clusterMaxZoom: 12.5,
    ...
    map.addLayer({
      id: "buzz-mobile-clusters",
      type: "circle",
      source: "buzz-mobile-venues",
      filter: ["has", "point_count"],
      paint: {
        "circle-color": ["step", ["get", "point_count"], "#FFD54A", 15, "#FF9F43", 40, "#FF5C5C"],
        "circle-radius": ["step", ["get", "point_count"], 16, 15, 20, 40, 24],
        "circle-stroke-width": 2,
        "circle-stroke-color": "rgba(255,255,255,.9)",
      },
    });
    map.addLayer({
      id: "buzz-mobile-cluster-count",
      type: "symbol",
      source: "buzz-mobile-venues",
      filter: ["has", "point_count"],
      layout: { "text-field": ["get", "point_count_abbreviated"], "text-size": 11, "text-font": ["DIN Offc Pro Bold", "Arial Unicode MS Bold"] },
      paint: { "text-color": "#101114" },
    });
    map.addLayer({
      id: "buzz-mobile-pin-halo",
      type: "circle",
      source: "buzz-mobile-venues",
      filter: ["!", ["has", "point_count"]],
      layout: { "circle-sort-key": ["case", ["==", ["get", "selected"], true], 10, 0] },
      paint: {
        "circle-radius": ["case", ["==", ["get", "selected"], true], 20, ["interpolate", ["linear"], ["get", "score"], 40, 8, 100, 16]],
        "circle-color": heatColor,
        "circle-opacity": ["case", ["==", ["get", "selected"], true], 0.32, 0.16],
        "circle-blur": 0.78,
      },
    });
    map.addLayer({
      ...
      layout: { "circle-sort-key": ["case", ["==", ["get", "selected"], true], 10, 0] },
      paint: {
        "circle-radius": ["case", ["==", ["get", "selected"], true], 10.5, ["interpolate", ["linear"], ["zoom"], 7, 5, 12, 6.5, 16, 8]],
        "circle-color": heatColor,
    ...
    map.addLayer({
      id: "buzz-mobile-score",
      type: "symbol",
      source: "buzz-mobile-venues",
      filter: ["!", ["has", "point_count"]],
      minzoom: 13.3,
      layout: { "text-field": ["to-string", ["get", "score"]], "text-size": 8.5, "text-font": ["DIN Offc Pro Bold", "Arial Unicode MS Bold"] },
      paint: { "text-color": "#FFFFFF" },
    });
    map.addLayer({
      id: "buzz-mobile-hitbox",
      type: "circle",
      source: "buzz-mobile-venues",
      filter: ["!", ["has", "point_count"]],
      paint: { "circle-radius": TAP_RADIUS_PX, "circle-color": "rgba(0,0,0,.01)", "circle-opacity": 0.01 },
    });
    ...
    const chooseFromTap = (event: mapboxgl.MapLayerMouseEvent) => {
      ...
      const unique = new Map<string, { id: string; distance: number }>();
      ...
      const repeated = previous && Date.now() - previous.at < 1400 && Math.hypot(previous.x - event.point.x, previous.y - event.point.y) < 20 && previous.ids.join("|") === ids.join("|");
      const index = repeated ? (previous.index + 1) % ids.length : 0;
    ...
    const expandCluster = (event: mapboxgl.MapLayerMouseEvent) => {
      ...
      (map.getSource("buzz-mobile-venues") as mapboxgl.GeoJSONSource).getClusterExpansionZoom(clusterId, (error, zoom) => {
        if (!error && zoom != null) map.easeTo({ center: feature.geometry.coordinates as [number, number], zoom, duration: 450 });
      });
    };
  ...
  async function chooseLocation(result: LocationResult) {
    ...
    if (result.bbox?.length === 4) {
      map?.fitBounds([[result.bbox[0], result.bbox[1]], [result.bbox[2], result.bbox[3]]], { padding: 42, maxZoom: 14, duration: 650 });
    } else {
      map?.easeTo({ center: [result.longitude, result.latitude], zoom: 13.5, duration: 650 });
    }
  ...
  return (
    <div className="buzz-mobile lg:hidden">
      <header className="buzz-mobile-header">
        <button className="buzz-mobile-brand" type="button" onClick={() => scrollRef.current?.scrollTo({ top: 0, behavior: "smooth" })} aria-label="Buzz home">
          <strong>BUZZ</strong><span>LIVE NEARBY</span>
        </button>
        <div className="buzz-mobile-actions">
          <button type="button" aria-label="Search places and areas" onClick={() => setSearchOpen(true)}><Search /></button>
          <button type="button" aria-label="Saved places" onClick={() => setFavoritesOpen(true)}><Bookmark /></button>
          <button type="button" className="buzz-mobile-avatar" aria-label="Open profile" onClick={() => setProfileOpen(true)}><UserRound /></button>
        </div>
      </header>
      ...
      <main ref={scrollRef} className="buzz-mobile-scroll">
        <section className="buzz-mobile-hero">
          ...
          {hottest && (
            <button type="button" className="buzz-best-move" onClick={() => openVenue(hottest)}>
              <span><small>BEST MOVE RIGHT NOW</small><strong>{hottest.name}</strong><em>{hottest.city || "Nearby"} · {statusFor(hottest)}</em></span>
              <b>{score(hottest)}<small>BUZZ</small></b>
            </button>
          )}
        </section>

        <nav className="buzz-category-rail" aria-label="Place categories">
          {categories.map(([label, Icon]) => (
            <button type="button" key={label} className={active === label ? "active" : ""} onClick={() => selectCategory(label)} aria-pressed={active === label}>
        ...
        <section className={`buzz-mobile-map mobile-native-map${mapMode ? " full" : ""}`} aria-label="Live Buzz map">
          ...
          {selected && !detailsOpen && (
            <div className="buzz-map-preview" role="button" tabIndex={0} onKeyDown={handlePreviewKey} onClick={() => setDetailsOpen(true)}>
              <div className="buzz-map-preview-photo">{selected.photoUrl ? <img src={selected.photoUrl} alt="" /> : selected.name.slice(0, 1)}</div>
              <div><span className="buzz-score-ring">{score(selected)}</span><strong>{selected.name}</strong><small>{milesLabel(selected.distanceMiles) || selected.city || "Nearby"} · {statusFor(selected)}</small><p>{selected.event?.name || selected.reason || "Popular nearby right now"}</p></div>
              <ChevronRight />
              <button type="button" className="buzz-preview-close" onClick={event => { event.stopPropagation(); setSelected(null); setSelectedVenueId(null); }} aria-label="Close venue preview"><X /></button>
            </div>
          )}
        </section>

        <button type="button" className="buzz-ai-card" onClick={() => { setActiveTab("ai"); setPlannerOpen(true); }}>
          <span><Sparkles /></span><div><strong>Not sure where to go?</strong><small>Tell Buzz the vibe, budget, and who’s coming.</small></div><b>Ask Buzz <ChevronRight /></b>
        </button>
          ...
          {filtered.length ? (
            <div className="buzz-feed-list">
              {filtered.map((venue, index) => {
                ...
                return (
                  <article className={selectedVenueId === venue.id ? "buzz-feed-row selected" : "buzz-feed-row"} key={venue.id} onClick={() => openVenue(venue)}>
                    <div className="buzz-feed-photo">{venue.photoUrl ? <img src={venue.photoUrl} alt="" /> : venue.name.slice(0, 1)}</div>
                    <div className="buzz-feed-copy"><strong>{venue.name}</strong><span><b>{statusFor(venue)}</b> · {trendPercent(venue)}% busier</span><small>{venue.event?.name ? `🎟 ${venue.event.name}` : venue.reason || "Popular nearby right now"}</small></div>
                    <div className="buzz-feed-meta"><span>{distanceOrRank}</span><div className="buzz-feed-score"><b>{score(venue)}</b><small>BUZZ</small></div><button type="button" className={favoriteIds.has(venue.id) ? "saved" : ""} onClick={event => toggleFavorite(event, venue)} aria-label={favoriteIds.has(venue.id) ? `Remove ${venue.name} from saved places` : `Save ${venue.name}`}><Heart fill={favoriteIds.has(venue.id) ? "currentColor" : "none"} /></button></div>
                  </article>
                );
              })}
            </div>
          ) : (
            <div className="buzz-empty"><Search /><strong>No {active.toLowerCase()} spots found</strong><p>Zoom out, search another area, or try another category.</p></div>
          )}
        </section>
      </main>

      {!mapMode && (
        <nav className="buzz-mobile-bottom" aria-label="Primary navigation">
          <button type="button" className={activeTab === "explore" ? "active" : ""} onClick={() => { setActiveTab("explore"); scrollRef.current?.scrollTo({ top: 0, behavior: "smooth" }); }}><span><Compass /></span><small>Explore</small></button>
          <button type="button" className={activeTab === "map" ? "active" : ""} onClick={() => { setActiveTab("map"); window.setTimeout(() => mapRef.current?.resize(), 120); }}><span><Map /></span><small>Map</small></button>
          <button type="button" className={activeTab === "favorites" ? "active" : ""} onClick={() => { setActiveTab("favorites"); setFavoritesOpen(true); }}><span><Heart /></span><small>Saved</small></button>
          <button type="button" className={activeTab === "alerts" ? "active" : ""} onClick={() => { setActiveTab("alerts"); setAlertsOpen(true); }}><span><Bell />{!alertsEnabled && <i />}</span><small>Alerts</small></button>
          <button type="button" className={activeTab === "ai" ? "active" : ""} onClick={() => { setActiveTab("ai"); setPlannerOpen(true); }}><span><Sparkles /></span><small>Ask Buzz</small></button>
        </nav>
      )}

      {detailsOpen && selected && (
        <div className="planner-backdrop" onClick={() => setDetailsOpen(false)}>
          <section className="utility-sheet buzz-detail-sheet" onClick={event => event.stopPropagation()}>
            <div className="planner-handle" />
            <div className="utility-head"><div><span>{statusFor(selected).toUpperCase()}</span><h2>{selected.name}</h2><p>{milesLabel(selected.distanceMiles) || selected.city || "Nearby"} · {selected.openNow === false ? "Closed" : "Open now"}</p></div><button type="button" onClick={() => setDetailsOpen(false)}><X /></button></div>
            <div className="buzz-detail-metrics"><div><small>BUZZ SCORE</small><strong>{score(selected)}</strong><p>{statusFor(selected)}</p></div><div><small>MOMENTUM</small><strong>+{trendPercent(selected)}%</strong><p>{selected.activity?.trendLabel || "Steady"}</p></div></div>
            <div className="alert-card"><Sparkles /><div><strong>Why the Buzz?</strong><p>{selected.event?.name ? `${selected.event.name} is driving activity. ` : ""}{selected.reason || "Strong activity and nearby interest."}</p></div></div>
            <div className="buzz-detail-actions"><button type="button" onClick={event => toggleFavorite(event, selected)}><Heart fill={favoriteIds.has(selected.id) ? "currentColor" : "none"} />{favoriteIds.has(selected.id) ? "Saved" : "Save"}</button><button type="button" onClick={() => toggleVenueAlert(selected)}><Bell />{selectedAlert ? "Watching" : "Alert at 80+"}</button></div>
            <button type="button" className="alert-action" onClick={() => window.open(`https://www.google.com/maps/dir/?api=1&destination=${selected.lat},${selected.lng}`, "_blank")}><Navigation /> Get directions</button>
          </section>
        </div>
      )}

      {searchOpen && (
        <div className="planner-backdrop" onClick={closeOverlays}>
          <section className="utility-sheet buzz-search-sheet" onClick={event => event.stopPropagation()}>
            <div className="planner-handle" />
            <div className="utility-head"><div><span>SEARCH ANYWHERE</span><h2>Find a place or area</h2><p>Venue, neighborhood, city, state, or ZIP code.</p></div><button type="button" onClick={closeOverlays}><X /></button></div>
            <form className="buzz-search-form" onSubmit={event => event.preventDefault()}><Search /><input autoFocus value={searchQuery} onChange={event => setSearchQuery(event.target.value)} placeholder="Try Oceanfront, 23451, Atlanta…" />{searchQuery && <button type="button" onClick={() => setSearchQuery("")}><X /></button>}</form>
            {searchLoading && <p className="buzz-search-empty">Searching places and locations…</p>}
            {!searchLoading && searchVenues.length > 0 && <div className="buzz-search-section"><span>PLACES</span><div>{searchVenues.map(venue => <button type="button" key={venue.id} onClick={() => { setSearchOpen(false); setVenues(current => current.some(item => item.id === venue.id) ? current : [venue, ...current]); openVenue(venue); }}><i>{venue.name.slice(0, 1)}</i><span><strong>{venue.name}</strong><small>{venue.city || venue.type || "Place"}</small></span><em>{score(venue)}</em></button>)}</div></div>}
            {!searchLoading && searchLocations.length > 0 && <div className="buzz-search-section"><span>AREAS & ZIP CODES</span><div>{searchLocations.map(result => <button type="button" key={result.id} onClick={() => void chooseLocation(result)}><i><MapPin /></i><span><strong>{result.name}</strong><small>{result.detail}</small></span><ChevronRight /></button>)}</div></div>}
            {!searchLoading && searchQuery.length >= 2 && !searchVenues.length && !searchLocations.length && <p className="buzz-search-empty">No match yet. Try a city, ZIP code, neighborhood, category, or venue name.</p>}
          </section>
        </div>
      )}

      {plannerOpen && (
        <div className="planner-backdrop" onClick={() => setPlannerOpen(false)}>
          <section className="planner-sheet" onClick={event => event.stopPropagation()}>
            <div className="planner-handle" />
            <div className="planner-head"><div><span>ASK BUZZ</span><h2>What kind of move?</h2><p>Describe the vibe, budget, people, or timing.</p></div><button type="button" onClick={() => setPlannerOpen(false)}><X /></button></div>
            <div className="planner-prompts">{prompts.map(prompt => <button type="button" key={prompt} onClick={() => void runPlanner(prompt)}>{prompt}</button>)}</div>
            <form onSubmit={submitPlanner} className="planner-form"><input value={plannerQuery} onChange={event => setPlannerQuery(event.target.value)} placeholder="Try chill date night near Norfolk" /><button type="submit" disabled={plannerLoading || !plannerQuery.trim()}>{plannerLoading ? <span className="planner-spinner" /> : <Send />}</button></form>
            {plannerError && <p className="planner-error">{plannerError}</p>}
            {plannerResults.length > 0 && <div className="planner-results"><div className="planner-result-title"><span>YOUR MOVE</span><strong>{plannerTitle}</strong></div>{plannerResults.map((venue, index) => <button type="button" key={venue.id} onClick={() => { setPlannerOpen(false); openVenue(venue); }}><i>{index + 1}</i><span><strong>{venue.name}</strong><small>{venue.event?.name || venue.reason || venue.city || "Recommended right now"}</small></span><ChevronRight /></button>)}</div>}
          </section>
        </div>
      )}
