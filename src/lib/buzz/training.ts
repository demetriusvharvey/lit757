import type { BuzzCalibrationModel } from "./calibration";
...
export function trainCalibrationModel(rows: CalibrationTrainingRow[], now = new Date()): BuzzCalibrationModel {
  ...
  clean.forEach((row, index) => {
    ...
    hourGroups[date.getHours()].push(error);
    dayGroups[date.getDay()].push(error);
    monthGroups[date.getMonth()].push(error);
  });
